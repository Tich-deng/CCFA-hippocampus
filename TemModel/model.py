...
class Model(torch.nn.Module):
    ...
    def forward(self, walk, prev_iter=None, prev_M=None):
        ...
        for g, x, a in walk:
            ...
            L, M, g_gen, p_gen, x_gen, x_logits, x_inf, g_inf, p_inf = self.iteration(x, g, steps[-1].a, steps[-1].M,
    ...
    def loss(self, g_gen, p_gen, x_logits, x, g_inf, p_inf, p_inf_x, M_prev):
        ...
        L_p_x = torch.sum(torch.stack(utils.squared_error(p_inf, p_inf_x), dim=0), dim=0) if self.hyper[
            'use_p_inf'] else torch.zeros_like(L_p_g).to(parameters.device)
    ...
    def init_trainable(self):
        ...
        self.alpha = torch.nn.ParameterList([torch.nn.Parameter(
            torch.tensor(np.log(self.hyper['f_initial'][f] / (1 - self.hyper['f_initial'][f])), dtype=torch.float).to(parameters.device)) for
                                             f in range(self.hyper['n_f'])])
        # Entorhinal preference weights
        self.w_x = torch.nn.Parameter(torch.tensor(1.0).to(parameters.device))
        # Entorhinal preference bias
        self.b_x = torch.nn.Parameter(torch.zeros(self.hyper['n_x_c']).to(parameters.device))
        # Frequency module specific scaling of sensory experience before input to hippocampus
        self.w_p = torch.nn.ParameterList([torch.nn.Parameter(torch.tensor(1.0).to(parameters.device)) for f in range(self.hyper['n_f'])])
        ...
        self.g_init = torch.nn.ParameterList([torch.nn.Parameter(
            torch.tensor(truncnorm.rvs(-2, 2, size=self.hyper['n_g'][f], loc=0, scale=self.hyper['g_init_std']),
                         dtype=torch.float).to(parameters.device)) for f in range(self.hyper['n_f'])])
        ...
        self.D_no_a = torch.nn.ParameterList([torch.nn.Parameter(torch.zeros(
            sum([self.hyper['n_g'][f_from] for f_from in range(self.hyper['n_f']) if
                 self.hyper['g_connections'][f_to][f_from]]) * self.hyper['n_g'][f_to]).to(parameters.device)) for f_to in
                                              range(self.hyper['n_f'])])
        ...
        self.MLP_mu_g_mem.set_weights(-1, [torch.tensor(
            truncnorm.rvs(-2, 2, size=list(self.MLP_mu_g_mem.w[f][-1].weight.shape), loc=0,
                          scale=self.hyper['g_mem_std']), dtype=torch.float).to(parameters.device) for f in range(self.hyper['n_f'])])
    ...
    def init_iteration(self, g, x, a, M):
        ...
        if M is None:
            ...
            M = [torch.zeros((self.hyper['batch_size'], sum(self.hyper['n_p']), sum(self.hyper['n_p'])),
                             dtype=torch.float).to(parameters.device)]
            ...
            if self.hyper['use_p_inf']:
                ...
                M.append(M[0] if self.hyper['common_memory'] else torch.zeros(
                    (self.hyper['batch_size'], sum(self.hyper['n_p']), sum(self.hyper['n_p'])), dtype=torch.float).to(parameters.device))
                # Initialise previous abstract location by stacking abstract location prior
        g_inf = [torch.stack([self.g_init[f] for _ in range(self.hyper['batch_size'])]).to(parameters.device) for f in
                 range(self.hyper['n_f'])]
        # Initialise previous sensory experience with zeros, as there is no data yet for temporal smoothing
        x_inf = [torch.zeros((self.hyper['batch_size'], self.hyper['n_x_f'][f])).to(parameters.device) for f in
                 range(self.hyper['n_f'])]
            ...
            for a_i, a in enumerate(prev_iter[0].a):
                ...
                if a is None:
                    ...
                    for f, x_inf in enumerate(prev_iter[0].x_inf):
                        x_inf[a_i, :] = torch.zeros(self.hyper['n_x_f'][f]).to(parameters.device)
    ...
    def inf_g(self, p_x, g_gen, x, locations):
        ...
        if any(shiny_envs):
            ...
            shiny_locations = torch.unsqueeze(torch.stack(
                [torch.tensor(location['shiny'], dtype=torch.float).to(parameters.device) for location in locations if
            ...
            for f in range(module_start, self.hyper['n_f']):
                ...
                mask = torch.zeros_like(mu_g[f], dtype=torch.bool).to(parameters.device)
    ...
    def f_mu_g_path(self, a_prev, g_prev, no_direc=None):
        ...
        if self.hyper['has_static_action']:
            ...
            a = torch.zeros((len(a_prev_step), self.hyper['n_actions'])).to(parameters.device).scatter_(1, torch.clamp(
                torch.tensor(a_prev_step).to(parameters.device).unsqueeze(1) - 1, min=0), 1.0 * (torch.tensor(a_prev_step).to(parameters.device).unsqueeze(1) > 0))
        else:
            # Without static actions: each action label should become a one-hot vector for that label
            a = torch.zeros((len(a_prev_step), self.hyper['n_actions'])).to(parameters.device).scatter_(1,
                                                                                  torch.tensor(a_prev_step).to(parameters.device).unsqueeze(
                                                                                      1), 1.0)
        # Get vector of transition weights by feeding actions into MLP        
        D_a = self.MLP_D_a([a.to(parameters.device) for _ in range(self.hyper['n_f'])])
    ...
    def f_c(self, decompressed):
        ...
        return torch.stack([self.hyper['two_hot_table'][i] for i in torch.argmax(decompressed, dim=1)], dim=0).to(
            parameters.device)
    ...
    def attractor(self, p_query, M, retrieve_it_mask=None):
        ...
        retrieve_it_mask = [torch.ones(sum(self.hyper['n_p'])).to(parameters.device) for _ in
...
class LSTM(torch.nn.Module):
    ...
    def forward(self, data, prev_hidden=None):
        ...
        if prev_hidden is None:
            hidden_state = torch.randn(self.lstm.num_layers, data.shape[0], self.lstm.hidden_size).to(parameters.device)
            cell_state = torch.randn(self.lstm.num_layers, data.shape[0], self.lstm.hidden_size).to(parameters.device)
    ...
    def prepare_data(self, data_in):
        ...
        actions = [torch.zeros((len(step[2]), self.n_a)).to(parameters.device).scatter_(1, torch.tensor(step[2]).to(parameters.device).unsqueeze(1), 1.0) for step
                   in data_in]
        ...
        return data.to(parameters.device)
...
class Iteration:
    ...
    def correct(self):
        ...
        predictions = [tensor.cpu().detach().numpy() for tensor in self.x_gen]
