...
if load_existing_model:
    ...
    params = torch.load(model_path + '/params_' + str(i_start) + '.pt').to(parameters.device)
...
for i in range(i_start, params['train_it']):
    ...
    for i_step, step in enumerate(chunk):
        chunk[i_step][1] = torch.stack(step[1], dim=0).to(parameters.device)

    # Forward-pass this walk through the network
    forward = tem(chunk, prev_iter)

    # Accumulate loss from forward pass
    loss = torch.tensor(0.0).to(parameters.device)
    ...
    for step in forward:
        ...
        step_loss = torch.tensor(0).to(parameters.device) if not step_loss else torch.mean(
            torch.stack(step_loss, dim=0), dim=0)
        # Save all separate components of loss for monitoring
        plot_loss = plot_loss + step_loss.cpu().detach().numpy()
    ...
    if i % 10 == 0:
        ...
        logger.info(
            'Loss: {:.2f}. <p_g> {:.2f} <p_x> {:.2f} <x_gen> {:.2f} <x_g> {:.2f} <x_p> {:.2f} <g> {:.2f} <reg_g> {:.2f} <reg_p> {:.2f}'.format(
                loss.cpu().detach().numpy(), *plot_loss))
        ...
        writer.add_scalar('Losses/Total', loss.cpu().detach().numpy(), i)
