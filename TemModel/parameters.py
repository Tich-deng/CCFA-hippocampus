...
def parameters():
    ...
    params['loss_weights'] = torch.tensor([params['loss_weights_p'], params['loss_weights_p'], params['loss_weights_x'], params['loss_weights_x'], params['loss_weights_x'], params['loss_weights_g'], params['loss_weights_reg_g'], params['loss_weights_reg_p']], dtype=torch.float).to(device)
    ...
    params['p_update_mask'] = torch.zeros((np.sum(params['n_p']),np.sum(params['n_p'])), dtype=torch.float).to(device)
    ...
    params['p_retrieve_mask_inf'] = [torch.zeros(sum(params['n_p'])).to(device) for _ in range(params['i_attractor'])]
    params['p_retrieve_mask_gen'] = [torch.zeros(sum(params['n_p'])).to(device) for _ in range(params['i_attractor'])]
    ...
    params['W_repeat'] = [torch.tensor(np.kron(np.eye(params['n_g_subsampled'][f]),np.ones((1,params['n_x_f'][f]))), dtype=torch.float).to(device) for f in range(params['n_f'])]
    ...
    params['W_tile'] = [torch.tensor(np.kron(np.ones((1,params['n_g_subsampled'][f])),np.eye(params['n_x_f'][f])), dtype=torch.float).to(device) for f in range(params['n_f'])]
    ...
    params['two_hot_table'] = [torch.tensor(code).to(device) for code in params['two_hot_table']]
    ...
    params['g_downsample'] = [torch.cat([torch.eye(dim_out, dtype=torch.float).to(device),torch.zeros((dim_in-dim_out,dim_out), dtype=torch.float).to(device)]) for dim_in, dim_out in zip(params['n_g'],params['n_g_subsampled'])]
    return params
...
def parameter_iteration(iteration, params):
    ...
    loss_weights = torch.tensor([L_p_g, L_p_x, L_x_gen, L_x_g, L_x_p, L_g, L_reg_g, L_reg_p]).to(device)
