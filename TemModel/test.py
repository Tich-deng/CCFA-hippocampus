...
date = '2025-03-01' # 2020-10-13 run 0 for successful node agent
run = '19'
index = '15000'
...
tem = model.Model(params).to(parameters.device)
...
for i_step, step in enumerate(model_input):
    model_input[i_step][1] = torch.stack(step[1], dim=0).to(parameters.device)
